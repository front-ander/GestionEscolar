// JavaScript para el módulo de Notas
...
// Función para validar formularios de notas
function validateNotaForm() {
    const valor = document.getElementById('valor');
    if (valor) {
        const notaValue = parseFloat(valor.value);
        if (notaValue < 0 || notaValue > 20) {
            showNotification('La nota debe estar entre 0 y 20', 'danger');
            return false;
        }
    }
    return true;
...
function updateStats() {
    const totalNotas = document.querySelectorAll('tbody tr').length;
    const statsNumbers = document.querySelectorAll('.stats-number');
    
    statsNumbers.forEach(stat => {
        stat.textContent = totalNotas;
    });
}
